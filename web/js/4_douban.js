...
Douban = new Object();
Douban.errdetail = [ "", "未知错误", "文件过大", "信息不全", "域名错误", "分类错误", "用户错误",
		"权限不足", "没有文件", "保存文件错误", "不支持的文件格式", "超时", "文件格式有误", "", "添加文件出错",
		"已经达到容量上限", "不存在的相册", "删除失败", "错误的MP3文件", "有禁用的内容,请修改重试" ];
	...
	$.postJSON_withck(c, {}, function(j) {
		if (j.result) {
			if (d) {
				...
				$("#voteuse_" + f).html(
						'<span class="m gtleft">你的投票已经提交，谢谢。</span>');
				$("#userate_" + f).html(
						'<p id="userate_%s" class="pl">' + j.usecount + "/"
								+ j.totalcount + "的人觉得此评论有用:</p>")
			}
		}
		return false
	})
	...
	$(b)
			.submit(
					function() {
						...
						$("#submits").hide().after(
								'<div id="saving" class="m">正在保存...</div>');
						refine_dialog();
						return false
					});
	if (h) {
						...
						$
								.getJSON(
										...
										function(e) {
											...
											populate_tag_btns("我的标签:", $(
													"#mytags", o), e.my_tags, n);
											populate_tag_btns("常用标签:", $(
													"#populartags", o),
													e.popular_tags, n);
											if (j == "pbtn" || j == "cbtn") {
	...
	$(b).click(
			function() {
				...
				$.getJSON(d, g && {
					...
					populate_tag_btns("我的标签(点击添加):", $("#mytags", j),
							l.my_tags, n);
					populate_tag_btns("豆瓣成员常用的标签(点击添加):", $("#populartags", j),
							l.popular_tags, n);
					if (f == "pbtn") {
		...
		.extend( {
			...
			hover_fold : function(d) {
				var b = {
					...
					return function() {
						$("img", e).attr("src", "/pics/arrow1_" + f + "0_.png")
					}
				};
			...
			setItemList : function(p) {
				var c = {}, g = "", n = '<img class="gray-loader" src="1_spinner.gif" />', e = "1_spinner.gif", j = ".input-create", l = {
					keyup : function(q) {
...
window.console
		&& window.console
				.info("喜欢看豆瓣的代码，还是发现了什么bug？不如和我们一起为豆瓣添砖加瓦吧！\nhttp://jobs.douban.com/#position-zsqd");