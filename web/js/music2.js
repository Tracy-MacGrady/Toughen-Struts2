...
Douban = new Object();
Douban.errdetail = [ "", "鏈煡閿欒", "鏂囦欢杩囧ぇ", "淇℃伅涓嶅叏", "鍩熷悕閿欒", "鍒嗙被閿欒",
		"鐢ㄦ埛閿欒", "鏉冮檺涓嶈冻", "娌℃湁鏂囦欢", "淇濆瓨鏂囦欢閿欒", "涓嶆敮鎸佺殑鏂囦欢鏍煎紡", "瓒呮椂",
		"鏂囦欢鏍煎紡鏈夎", "", "娣诲姞鏂囦欢鍑洪敊", "宸茬粡杈惧埌瀹归噺涓婇檺", "涓嶅瓨鍦ㄧ殑鐩稿唽", "鍒犻櫎澶辫触",
		"閿欒鐨凪P3鏂囦欢", "鏈夌鐢ㄧ殑鍐呭,璇蜂慨鏀归噸璇�" ];
var trace = function(b) {
	if (!/^http:\/\/(www|movie|music\.|book|douban\.fm)/.test(location.href)
...
var remote_submit_json = function(f, e, c, g) {
	...
	if (c === false) {
		$(":submit,:input", f).removeAttr("disabled")
	} else {
		$(":submit,:input", f).attr("disabled", true)
	}
	var b = g || f.action;
...
Douban.init_evc = function(c) {
	...
	event_monitor.subscribe("entry_" + b + "_voted", function(d, f) {
		var e = f.rec_count;
		if (e) {
			$(c).text("" + e + "浜烘帹鑽�").removeClass("hidden")
		}
	})
};
	...
	$.postJSON_withck(c, {}, function(j) {
		if (j.result) {
			if (d) {
				...
				if ((h.text() == j.usecount)
						&& (g.text() == j.totalcount - j.usecount)
						&& (j.result != "notself")) {
					alert("浣犲凡缁忔姇杩囩エ浜�")
				}
				h.html(j.usecount);
				...
				$("#voteuse_" + f).html(
						'<span class="m gtleft">浣犵殑鎶曠エ宸茬粡鎻愪氦锛岃阿璋€�</span>');
				$("#userate_" + f).html(
						'<p id="userate_%s" class="pl">' + j.usecount + "/"
								+ j.totalcount + "鐨勪汉瑙夊緱姝よ瘎璁烘湁鐢�:</p>")
			}
		}
		return false
	})
...
Douban.init_bef = function(h) {
	var b = $(h).attr("id").split("entry-")[1], g = $(".unfolder", h), d = $(
			".folder", h), c = $(".entry-summary", h), e = $(".entry-full", h);
	g.click(function() {
		if (e.text() == "") {
			var j = $('<div class="loadtip">姝ｅ湪杞藉叆...</div>');
	...
	var f = function(l) {
		...
		if (k.length) {
			...
			if (d.length) {
				...
				g
						.after(d = $('<ul id="vsl" class="user-list pl indent"></ul>'));
				d.append(t)
			}
...
Douban.init_closelink = function(b) {
	$('<a href="#">鍏抽棴</a>').appendTo($(b)).click(function() {
		window.close();
		return false
	})
};
	...
	if (/recc/.test(f.name)) {
		...
		if (/sayc/.test(f.name)) {
			...
			f.click(function() {
				...
				g = (g.slice(0, 1) == "!") ? g.slice(1) : "鐪熺殑瑕�" + g + "?";
				return confirm(g)
			})
		}
	}
...
var populate_tag_btns = function(g, h, b, e) {
	if (b.length) {
		...
		$.each(b, function(k, d) {
			var j = $('<span class="tagbtn"></span>').addClass(
					e[d.toLowerCase()] ? "rdact" : "gract").text(d);
			f.append(j).append(" &nbsp; ")
		});
		c.append(f);
		h.append(c)
...
Douban.init_music_sync_form = function(c) {
	...
	if (e.length && d.length) {
		$("#overlay, #dialog").hide();
		var b = dui.Dialog( {
			title : "鎺堟潈鍚屾淇℃伅鑷宠眴鐡ｈ涓庣涓夋柟缃戠珯",
			url : "/settings/pop",
			autoupdate : true,
			callback : function(f, g) {
				...
				$("a#btn-never", g.node).bind(
						"click",
						function(h) {
							h.preventDefault();
							$.post_withck("/settings/never_pop_sync_settings",
									{}, function() {
										$("div.dui-dialog").remove();
										$("#overlay, #dialog").show()
									});
...
Douban.init_interest_form = function(b) {
	...
	var l = function() {
		var n = $("#inp-private"), o = n.parents("form").find(".share-label");
			...
			.submit(
					function() {
						...
						remote_submit_json(
								this,
								function(s) {
									var w = $("#dialog .shuo :input[type=checkbox]"), o = $("div#dialog form.movie-sns"), q = $("div#dialog form.book-sns");
									if (s.r != 0) {
										$("#saving").remove();
										$("#submits").show();
										$("#error").html(Douban.errdetail[s.r]);
										refine_dialog();
										return
									}
									...
									if (w.length && w[0].checked) {
										...
										if (o.length && s.cid) {
											...
											var u = dui
													.Dialog( {
														title : "宸蹭繚瀛樹负鎯崇湅",
														url : "/j/coupon_info?coupon_id="
																+ v,
														autoupdate : true,
														callback : function(x,
																y) {
															$(
																	"a#btn-giveup, a#btn-close",
																	y.node)
																	.bind(
																			"click",
																			function() {
																								...
																								function() {
																									...
																									A
																											.find(
																													".coupon_tit")
																											.html(
																													"棰嗗彇鎴愬姛锛屽彲浠ュ埌鍙敤鐨勪紭鎯犲埜 ( <a href='http://movie.douban.com/ticket/coupon/' target='_blank'>http://movie.douban.com/ticket/coupon/</a> ) 鏌ョ湅");
																									A
																											.find(
																													".coupon")
																											.css(
																...
																$(
																		...
																		.bind(
																				...
																				function(
																					...
																					$
																							.post_withck(
																									"/settings/never_pop_sync_settings",
																									{},
																									function() {
																										$(
																												"div.dui-dialog")
									...
									if ($(b).data("reload")) {
										if (/subject\/\d+\/comments/
												.test(location.href)) {
											location.href = location.href
													.split("?sort")[0]
													+ "?sort=time"
										} else {
											if (/people\/[^\/]+\/(edittag|all|do|wish|collect)/
													.test(location.href)) {
												location.href = location.href
											} else {
												if (location.search) {
						...
						$("#submits").hide().after(
								'<div id="saving" class="m">姝ｅ湪淇濆瓨...</div>');
						refine_dialog();
						return false
					});
	if (h) {
		$("#inp-private").click(l);
		$("input[name=share-shuo]").bind("click", {
			key : "shuo-conf"
		}, f);
		$("input[name=share-sina]").bind("click", {
			key : "sina-conf"
		}, f);
		$("input[name=share-tencent]").bind("click", {
			key : "tencent-conf"
		}, f)
	}
	...
	$(".tagbtn", b).each(function(o) {
		...
		if (d[n]) {
			...
			function() {
				var n = $(this).text();
				var p = $.trim(b.tags.value).split(" "), s = false, o = n
						.toLowerCase(), q;
...
Douban.init_stars = function(e) {
	var b = {
		...
		4 : "鎺ㄨ崘",
		5 : "鍔涜崘"
	}, g = $("#n_rating", e), c = $("#stars img", e), d = function(f) {
		var h = g.val() || 0;
		c.each(function(l) {
			var k = this.src.replace(/\w*\.gif$/, ((l < f) ? "sth"
					: ((l < h) ? "st" : "nst"))
					+ ".gif");
			this.src = k
		});
...
Douban.init_tries_to_listen = function(c) {
	...
	$(c).click(
			function() {
				...
				window.open($(c).attr("href"), "", "width=" + d + ",height="
						+ f + ",top=" + g + ",left=" + j
	...
	$(c).submit(
			function(e) {
				...
				if (d == "event") {
					...
					if (d == "group") {
						$("#discover_s").attr("action",
								"/group/search?q=" + $("#discover_text").value)
					} else {
...
var set_cookie = function(h, g, e, f) {
	...
	for ( var d in h) {
		document.cookie = d + "=" + h[d] + b + "; domain="
				+ (e || "douban.com") + "; path=" + (f || "/")
	}
};
...
Douban.init_more2 = function(b) {
	$(b).click(
			function() {
				...
				a.attr("value", "");
				$(this).before(
						'<br/><span class="pl idx">' + (nid + 1) + "</span>")
						.before(a);
				a.attr("id", id).attr("name", id).removeClass("m").wrap(
						"<span></span>");
				init_keyup();
				list_data[id] = "鏃�"
			})
						...
						$(this)
								.click(
										function() {
											...
											if (urls[1] != undefined) {
												$(
														'<input type="hidden" name="cat" value="' + urls[1] + '" />')
														.appendTo($("#ssform"))
											}
											$("#ssform").submit();
											return false
						...
						"/j/subject_suggest",
						{
							onSelect : function() {
								$(this)
										.parents("form")
										.append(
												'<span><input name="add" value="1" type="hidden"/></span>')
										.submit()
							}
						})
	}
...
this.show_dialog = function(c, b) {
	...
	$("body")
			.prepend(
					'<div id="overlay"></div><div id="dialog" style="width:' + (b || 550) + 'px;"></div>');
	if (c != null) {
		$("#dialog").html(c)
	} else {
		$("#dialog").html("<div class='loadpop'>姝ｅ湪杞藉叆锛岃绋嶅€�...</div>")
	}
	set_overlay()
};
this.set_overlay = function() {
	var c = $("#dialog"), d = c[0].offsetHeight + ($.browser.msie ? -2 : 16), b = c[0].offsetWidth + 16;
	$("#overlay").css( {
		height : d,
		width : b,
		marginLeft : -(b / 2) + "px",
...
(function() {
	...
	window.localStorage = function() {
		var e = document;
		if (!e.documentElement.addBehavior) {
			throw "don't support localstorage or userdata.";
			return
		}
		var c = "_localstorage_ie";
		...
		var b = function(f) {
			return function() {
				...
				d.expires = h.toUTCString();
				d.load(c);
				var g = f.apply(d, arguments);
				e.body.removeChild(d);
				return g
			}
		};
		return {
			getItem : b(function(f) {
				return this.getAttribute(f)
			}),
			setItem : b(function(f, g) {
				this.setAttribute(f, g);
				this.save(c)
			}),
			removeItem : b(function(f) {
				this.removeAttribute(f);
				this.save(c)
			}),
			clear : b(function() {
				var g = this.XMLDocument.documentElement.attributes;
				for ( var h = 0, f; f = g[h]; h++) {
					this.removeAttribute(f.name)
				}
				this.save(c)
			})
		}
...
$(window).one(
		"load",
		function() {
			var b = localStorage.getItem("report");
			if (!b) {
				return
			}
			b = b.split("_moreurl_separator_");
			var c = function(d) {
				if (d == "") {
					c(b.shift());
					return
				}
				$.get(typeof _MOREURL_REQ == "undefined" ? "/stat.html?" + d
						: _MOREURL_REQ + "?" + d, function() {
					if (b.length) {
						c(b.shift());
						localStorage.setItem("report", b
								.join("_moreurl_separator_"));
						return
					}
...
function _moreurl(b, e) {
	...
	if (window._SPLITTEST) {
		d.push("splittest=" + window._SPLITTEST)
	}
	localStorage.setItem("report", (localStorage.getItem("report") || "")
			+ "_moreurl_separator_" + d.join("&"))
}
	...
	$(b)
			.click(
					function(h) {
						...
						show_dialog(null);
						var f = $(this).attr("name").split("-"), j = f[0], c = f[1], k = f[2], g = f[3], d = "/j/subject/"
								+ c
								+ "/interest?"
								+ (k ? "interest=" + k : "")
								+ (g ? "&rating=" + g : "")
						...
						$
								.getJSON(
										...
										function(e) {
											...
											populate_tag_btns("鎴戠殑鏍囩:", $(
													"#mytags", o), e.my_tags, n);
											populate_tag_btns("甯哥敤鏍囩:", $(
													"#populartags", o),
													e.popular_tags, n);
											if (j == "pbtn" || j == "cbtn") {
											...
											if ($("#left_n").length) {
												var p = $("#left_n").text();
												llen = (p.match(/\d+/i) == p) ? p
														: 140;
												$("#comment").display_limit(
														llen, $("#left_n"))
											}
											if (j == "cbtn") {
												var v = $("h2", "#dialog");
												v.text(v.text().replace("淇敼",
														"鍐欑煭璇�"));
												if (!l[0].checked && l[1]) {
													l[1].checked = true
												}
	...
	$(b).click(
			function() {
				...
				$.getJSON(d, g && {
					...
					populate_tag_btns("鎴戠殑鏍囩(鐐瑰嚮娣诲姞):", $("#mytags", j),
							l.my_tags, n);
					populate_tag_btns("璞嗙摚鎴愬憳甯哥敤鐨勬爣绛�(鐐瑰嚮娣诲姞):", $(
							"#populartags", j), l.popular_tags, n);
					if (f == "pbtn") {
						$("form", j).data("reload", 1)
					}
					$("#collect_form_" + c).html("").append(
							'<p class="ul"></p>').append(j);
...
Douban.init_rec_btn = function(g) {
	var c = $(g).attr("name").split("-"), b = "/j/recommend", e = "rdialog-"
			+ c[1] + "-" + c[2], d = function() {
		var h = ((c[1] == "I") && (c[2] == undefined)) ? $("input", $(g)
				.parent())[0].value : c[2], j = (c[3] == undefined) ? "" : c[3], f = function(
				l) {
			if (l == "I") {
				var k = $(".text", "#dialog");
				if (k.length) {
			...
			if ($(g).hasClass("novote")) {
				$("form", "#dialog").append(
						'<input name="novote" value="1" type="hidden"/>')
			}
		};
		if ($("#" + e).length) {
			show_dialog($("#" + e).html());
			...
			.submit(function(d) {
				...
				remote_submit_json(
						...
						function(e) {
							trace(e);
							if (e.ban) {
								$(":submit,:input", b).removeAttr("disabled");
								$(".recsubmit")
										.before(
												'<div class="attn" style="text-align:center" id="ban_word">浣犵殑鎺ㄨ崘涓湁琚姝㈢殑鍐呭</div >');
								return
							}
							$("#dialog").html(
									'<div class="loadpop m">鎺ㄨ崘宸叉彁浜�</div>');
							set_overlay();
				...
				if (d.rev != "unfold") {
					...
					if (d.rel != "polling") {
						...
						$.getJSON(b, {
							...
							$('<div class="recreplylst"></div>').insertAfter(
									$(d).parent().parent()).html(e.html);
							load_event_monitor($(d).parent().parent().next());
							$(d).attr("rev", $(d).html()).text("闅愯棌鍥炲簲");
							d.rel = ""
						})
					}
				}
					...
					function() {
						remote_submit_json(
								...
								function(f) {
									...
									load_event_monitor(e);
									if (c == "n") {
										var d = $('<span><a href="javascript:void(0)">娣诲姞鍥炲簲</a></span>')
									} else {
										var d = $('<span style="margin-left:53px"><a href="javascript:void(0)">娣诲姞鍥炲簲</a></span>')
									}
									$("form", e).hide().after(d);
									d.click(function() {
										$(this).prev().show();
...
Douban.init_video_comment = function(b) {
	$(b).submit(function() {
		remote_submit_json(this, function(d) {
			...
			$("textarea", b).removeAttr("disabled").val("")
...
Douban.init_editable = function(f) {
	...
	var b = function(g) {
		if (g != undefined) {
			d.text(g);
			if (d.text() == "") {
				c.text("鐐瑰嚮娣诲姞鎻忚堪").addClass("sign-text")
			} else {
				c.text("淇敼").removeClass("sign-text")
			}
		}
		d.show();
		$(e).hide();
	...
	$("#edi", f).click(function() {
		$("#display, #edi").hide();
		$("input,textarea", e).removeAttr("disabled");
...
Douban.init_prompt_link = function(b) {
	$(b).click(
			function() {
				var c = prompt(b.title || "璇疯緭鍏�");
				if (c) {
					location.href = b.href
							+ (b.href.indexOf("?") == -1 ? "?" : "&") + b.name
...
$.post_withck = function(b, e, f, c, d) {
	if ($.isFunction(e)) {
		c = f;
		f = e;
		e = {}
		...
		.extend( {
			...
			insert_caret : function(c) {
				...
				if (document.all && k.createTextRange && k.p) {
					var j = k.p;
					j.text = j.text.charAt(j.text.length - 1) == "" ? c + ""
							: c
				} else {
			...
			get_sel : function() {
				var b = this[0];
				return document.all && b.createTextRange && b.p ? b.p.text
						: b.setSelectionRange ? b.value.substring(
			...
			multiselect : function(d) {
				var g = function() {
					return true
				}, f = d.onselect || g, e = d.onremove || g, c = d.onchange
						|| g, h = d.selclass || "sel", b = d.values || [];
				return this.click(function() {
					var k = /id(\d*)/.exec(this.className)[1], j = $.inArray(k,
							b);
					if (j != -1) {
						if (!e(this)) {
							return
			...
			setItemList : function(p) {
				var c = {}, g = "", n = '<img class="gray-loader" src="/pics/spinner.gif" />', e = "/pics/spinner.gif", j = ".input-create", l = {
					keyup : function(q) {
				...
				}, d = document.body, k = new Image(), f = {
					create : {
						title : "鏂板垎缁�",
						tips : "鍒涘缓鏂板垎缁�"
					}
				}, p = $.extend(f, p), b = '<span class="create-new">'
						+ p.create.title + "</span>", h = '<input class="input-create" type="text" value="" title="'
						+ p.create.tips
						+ '" maxlength="'
						+ p.create.maxLen
						+ '" />';
				...
				$(CSS_SET_GROUP_LIST)
						.delegate(
								...
								function(w) {
									w.preventDefault();
									var v = w.target, q = this, u = v.type === "checkbox" ? true
											: false, o = $(this).children(
											"input"), s = $(this).children(
											"input").val(), r = (u
											&& o.attr("checked") || !u
											&& !o.attr("checked")) ? "addtotag"
											: "removefromtag";
...
var check_form = function(b) {
	...
	$(":input", b)
			.each(
					function() {
						if ((/notnull/.test(this.className) && this.value == "")
								|| (/most/.test(this.className) && this.value && this.value.length > /most(\d*)/
										.exec(this.className)[1])) {
							$(this).next().show();
							c = false
						} else {
...
function center(b) {
	return {
		left : (document.documentElement.offsetWidth - b.offsetWidth) / 2
				+ "px",
		top : (document.documentElement.clientHeight - b.offsetHeight) * 0.45
				+ "px"
	}
}
...
pop_win.load = function(c, b) {
	pop_win('<div style="padding:20px 60px;">鍔犺浇涓�, 璇风◢绛�...</div>');
	$.ajax( {
		url : c,
		success : pop_win,
		cache : b || false,
...
function exp_dialog(b) {
	...
	return 0 - parseInt(b.offsetHeight / 2)
			+ (TBWindowMargin = c && c.scrollTop || document.body.scrollTop)
			+ "px"
}
function exp_overlay(b) {
	return 0
			- parseInt(b.offsetHeight / 2)
					...
					if ($(d).val().length < 50) {
						...
						$
								.getJSON(
										...
										function(f) {
											if (f.has) {
												if (confirm("灏戜簬50瀛楃殑璇勮灏嗚鑷姩杞负绠€鐭瘎璁恒€傚苟鏇挎崲涔嬪墠鍙戣〃鐨勭畝鐭瘎璁哄唴瀹广€傛槸鍚︾户缁紵")) {
													c.submit()
												}
											} else {
												c.submit()
...
Douban.init_get_more = function(b) {
	$(b).click(
			function() {
				...
				$(".a_switch_tab").removeClass("current");
				$(b).addClass("current");
				tag = b.name.replace("[", "", "g").replace("]", "", "g")
						.replace("'", "", "g");
				$("#tag-loader").attr("class", "loading").text("");
				$.getJSON(url, {
					tag : tag,
							...
							$.post_withck(url, {
								...
								if (d == "Y") {
									self_item.fadeOut(function() {
										$.getJSON($("#hide_full_path").attr(
												"name")
												+ "/switch", {
											tag : tag,
											perpage : total
										}, function(e) {
											$(".rec-list").replaceWith(
													e.subjects);
											load_event_monitor(".rec-list");
											$("#tag-loader").attr("class",
													"not-loading")
										})
									})
								}
...
Douban.init_post_link = function(b) {
	$(b)
			.click(
					function(j) {
						var d = $(this), c = d.attr("href"), p = d
								.attr("title")
								|| d.text() + "?", g = d.attr("rel"), q = g == "confirm_direct"
								|| g == "", h = g == "direct"
								|| g == "confirm_direct", k = d.attr("target"), n = c
								.split("?")[0], o = {}, l = c.split("?")[1]
								|| [];
						if (typeof l === "string") {
							l = l.split("&")
						}
						...
						if (h) {
							...
							for (i = 0, f; i < l.length; i++) {
								f = l[i].split("=");
								r.push('<input type="hidden" name="' + f[0]
										+ '" value="'
										+ unescape(f[1]).escapeHTML() + '">')
							}
							$(
									'<form action="' + n
											+ '" method="POST" target="'
...
Douban.init_donate = function() {
	var p = '<div class="blocktip dou-tip">{BODY}</div>', l = '<form action="" method="post"><div class="frm-item">浣犲皢鍚戜綔鑰呰禒閫�<b>1</b>棰楀皬璞�</div><div class="frm-item"><label for="dou-inp-msg">椤哄甫鎹庝釜璇�...</label><input id="dou-inp-msg" type="text" name="note"></div><div class="frm-submit"><span class="bn-flat"><input type="submit" value="閫佸嚭"></span><a href="#" class="tip-bn-cancel">鍙栨秷</a></div></form>', c = '<p>鈥滄劅璋⑩€濆皢鍚戜綔鑰呰禒閫�<b>1</b>棰楀皬璞嗭紝浣犺繕娌℃湁灏忚眴銆�<br><a href="http://www.douban.com/help/account#t4-q1">鎬庢牱鑾峰彇灏忚眴?</a></p><span class="bn-flat"><input type="button" class="tip-bn-cancel"  value="鐭ラ亾浜�"></span>', k = '<span class="donated-fail">{MSG}</span>', q = '<span class="donated-success">{MSG}</span>', d = "<p>澶勭悊涓紝璇风◢鍊�...</p>", f = ".tip-bn-cancel", e = "processing", j = function(
			s, u) {
		s.replaceWith(k.replace("{MSG}", u));
		b()
	}, h = function(s, w) {
		b();
		var v = $(p.replace("{BODY}", s)).appendTo("body"), z = w.offset(), u = [], y = $(window), x = y
				.scrollTop()
				+ y.height();
		...
		var y = v.offset(), s = [], x = $(window), w = x.scrollTop()
				+ x.height();
	...
	$("body")
			.delegate(
					...
					"click",
					function(z) {
						var B = $(z.currentTarget), s = B.attr("href").split(
								"?"), w, u, x, y, v = {
							elm : B
						}, A = {
							is_first : 1
...
window.console
		&& window.console
				.info("鍠滄鐪嬭眴鐡ｇ殑浠ｇ爜锛岃繕鏄彂鐜颁簡浠€涔坆ug锛熶笉濡傚拰鎴戜滑涓€璧蜂负璞嗙摚娣荤爾鍔犵摝鍚э紒\nhttp://jobs.douban.com/#position-zsqd");